...
DEMO_EXAMPLES = [
    ...
    {"text": "I keep seeing a server error when I save changes.", "label": "technical_issue"},
    {"text": "The dashboard is stuck loading after login.", "label": "technical_issue"},
    {"text": "I am locked out and the reset email never arrived.", "label": "account_access"},
    {"text": "My account was disabled and I cannot sign in.", "label": "account_access"},
    ...
    {"text": "We need export to CSV in the reporting page.", "label": "feature_request"},
    ...
    {"text": "The tracking page says delivered but nothing arrived.", "label": "shipping"},
...
def write_dataset_split(data_dir: Path, examples: list[dict[str, str]]) -> None:
    ...
    (processed_dir / "train.json").write_text(json.dumps(train, indent=2), encoding="utf-8")
    (processed_dir / "validation.json").write_text(json.dumps(validation, indent=2), encoding="utf-8")
    (processed_dir / "test.json").write_text(json.dumps(test, indent=2), encoding="utf-8")
